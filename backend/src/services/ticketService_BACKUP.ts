...
export const generateTicket = async (bookingId: string): Promise<{ ticketId: string; pdfBuffer: Buffer }> => {
  ...
  if (existingTickets.length > 0) {
    ...
    console.log('Creating ticket:', { ticketId, bookingId, ticketNumber });

    // Save ticket to database (without QR URL - will generate in PDF)
    await query<ResultSetHeader>(
      `INSERT INTO tickets (
        ticket_id, booking_id, qr_code_data, ticket_number, is_used, created_at
      ) VALUES (?, ?, ?, ?, FALSE, NOW())`,
